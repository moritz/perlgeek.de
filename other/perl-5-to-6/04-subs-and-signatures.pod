...
The last example uses the so-called I<colon pair syntax>. Leaving off the name
results in the value being C<True>, and negating it results in the value being
C<False>:
...
C<"abc."substr(@indexes.end)> in the first place).
