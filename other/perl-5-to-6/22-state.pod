...
It passes a bit more than 10,000 tests from the official test suite (May 2009).
Its progress in the test suite is monitored
on L<http://rakudo.org/status> .
...
C<YAML::XS> and C<Moose>):
    ...
    cd src/perl6/
    make
    make testt      # warning: takes lot of 80 time
    ./tryfile $your_file
...
Rakudo:  L<http://www.rakudo.org/>, L<http://www.parrot.org/>,
