...
is parsed as a call to sub C<if>. You can disambiguate with whitespaces:
...
C<use> or C<if>, but they are only special if used in a special syntax.
...
L<http://perlcabal.org/syn/S02.html#Literals>
