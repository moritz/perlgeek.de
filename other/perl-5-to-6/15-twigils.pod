...
Perl 6, there's a concept named I<seld-declared positional parameter>, and
...
And to keep the symetry between positional and named arguments, the C<:> sigil
does the same for named parameters, so these lines are roughly equivalent:

  my $block = { say $:stuff }
  my $sub   = sub (:$stuff) { say $stuff }

The C<?> sigil stands for variables and constants that are known at compile
time, like C<$?LINE> for the current line number (formerly C<__LINE__>), and
C<$?DATA> is the file handle to the C<DATA> section. 

Global variables can be accessed with the C<*> sigil, so C<%GLOBAL::ENV> can
...
other hand they remove the need for specical cases. In the case of object
