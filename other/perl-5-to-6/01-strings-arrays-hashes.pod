...
=head1 SYNOPSIS
    ...
    # (it tries to call the subroutine bar(), which is not defined
...
This means that you don't need special dereferencing syntax, and that you can
create objects that can act as arrays, hashes and subs at the same time.

Note that Rakudo only implements interpolation of scalars, closures and
backslash sequences at the moment; arrays and hashes are never interpolated
