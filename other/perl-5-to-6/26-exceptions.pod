...
Exceptions are generated either by implicit errors (for example
calling a non-existing method, or type check failures) or by explicitly
...
C<try> without a CATCH block the try block returns C<Any>).
...
Adding a C<CONTROL { ... }> block to the scope in which C<$block> is called
causes it to catch the control exception.
