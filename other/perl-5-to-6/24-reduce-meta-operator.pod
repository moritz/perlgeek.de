...
This is a very powerful construct that promotes the plus C<+> oeprator into a
...
Like all other operators whitespaces are forbidden, so you while you can write
C<[+]>, you can't say C<[ + ]>.
...
Note that C<[...]> always returns a scalar, so C<[,] @list> is actually the
same as C<[@list]>.
...
C<< @list.reduce(&infix:<+>) >>), and takes care of the associativity of the
operator.

If you're not convinced, play a bit with it (pugs mostly implements it), it's
real fun.

=head1 SEE ALSO

L<http://perlcabal.org/syn/S03.html#Reduction_operators>,
