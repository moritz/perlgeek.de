...
a meta operator, which combines zip with C<< Z=> >> (the Pair construction
...
generate code that reproduces the original data structure as good as possible.

C<:$var> generates a Pair ("I<colonpair>"), using the variable name as key
