...
"Perl 5 to 6" Lesson 12 - Lazyness
...
Lazyness allows you to deal with infinite lists: as long as you don't do
...
kills lazyness - if the list is infinite, it will likely loop infinitly.

In general all conversions to a scalar (like C<List.join>) are I<eager>, i.e.
non-lazy.

Lazyness prevents unnecessary computations, and can therefor boost performance
...
starts iterating. With lazyness that's not an issue:
...
=head2 Controlling Lazyness

Lazyness has its problems (and when you try to learn Haskell you'll notice how
...
In general lazyness makes programming easier because you don't have to know if
...
L<http://perlcabal.org/syn/S02.html#Lists|S02>
